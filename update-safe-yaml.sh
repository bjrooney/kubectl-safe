...
get_sha256() {
    local platform="$1"
    local extension="$2"
    local filename="kubectl-safe-$platform.$extension"
...
# Linux AMD64
LINUX_AMD64_SHA=$(get_sha256 "linux-amd64" "tar.gz")
...
# Linux ARM64
LINUX_ARM64_SHA=$(get_sha256 "linux-arm64" "tar.gz")
...
# Darwin AMD64
DARWIN_AMD64_SHA=$(get_sha256 "darwin-amd64" "tar.gz")
...
# Darwin ARM64
DARWIN_ARM64_SHA=$(get_sha256 "darwin-arm64" "tar.gz")
...
# Windows AMD64
WINDOWS_AMD64_SHA=$(get_sha256 "windows-amd64" "zip")
sed -i "/os: windows/,/arch: amd64/{
    s|uri: .*|uri: $BASE_URL/kubectl-safe-windows-amd64.zip|
    s|sha256: .*|sha256: \"$WINDOWS_AMD64_SHA\"|
}" safe.yaml

# Windows ARM64
WINDOWS_ARM64_SHA=$(get_sha256 "windows-arm64" "zip")
sed -i "/os: windows/,/arch: arm64/{
    s|uri: .*|uri: $BASE_URL/kubectl-safe-windows-arm64.zip|
    s|sha256: .*|sha256: \"$WINDOWS_ARM64_SHA\"|
}" safe.yaml
...
echo "  Windows ARM64: $WINDOWS_ARM64_SHA"
echo ""
